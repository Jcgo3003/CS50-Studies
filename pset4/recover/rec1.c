...
int main ( int argc, char *argv[])
{
    ...
    while ( fread(buffer, 512, 1 , inptr)  )///cada que va haciendo este while esta metiendo datos en el buffer
    {
        ...
        if ( buffer[0]==0xff && buffer[1]==0xd8 && buffer[2]==0xff && (buffer[3]&0xf0) == 0xe0 )
        {
            ...
            fwrite (buffer, 512, 1, outprt);///Writing the root512 block
            sen = 1;
            sen2 = 1;
            
            fread(buffer, 512, 1 , inptr);//movin
            
                while ( buffer[0]!=0xff && buffer[1]!=0xd8 && buffer[2]!=0xff && (buffer[3]&0xf0) != 0xe0 ) //kepping writing
                {
                ///este while deberia seguir hasta que pase que hay una nueva imagen pero salta mucho antes    
                    ...
                    if(sen2 == 1)
                    {   
                        fwrite(buffer, 512, 1, outprt);///Writing the next 512 block
                        sen2 = 0;
                    }
                    
                fread(buffer, 512, 1 , inptr);////reading next block
                fwrite(buffer, 512, 1, outprt);///Writing the next 512 block
                printf(" %i ", count);
...
//         if ( ( buffer[0]==0xff && buffer[1]==0xd8 && buffer[2]==0xff && (buffer[3]&0xf0) == 0xe0 ) || sen == 3 )
//         {
...
//             fwrite (buffer, 512, 1, outprt);///Writing the root512 block
//             printf(" count IF %i ", count);
            
//             sen = 1;
            
//             while ( sen == 1) //kepping writing
//             {
//             fread(buffer, 512, 1 , inptr);////reading next block
//             fwrite(buffer, 512, 1, outprt);///Writing the next 512 block
//             printf(" %i ", count);
...
//             }
//             ////ahora si cuenta hasta 50 pero so cierra el ultimo archivo
//         }
